...
RECOMP_EXPORT s32 AudioApi_IsSequencePlaying(s32 seqId) {
    ...
    /*
     * Fallback: sequence flags may be custom/overridden for replacement tracks.
     * Some game logic (e.g. Bremen/Kamaro checks) only needs to know if a
     * specific seqId is active at all, regardless of which player currently owns it.
     */
    for (i = 0; i < SEQ_PLAYER_MAX; i++) {
        if (seqId == AudioApi_GetActiveSeqId(i)) {
            return true;
        }
    }
...
RECOMP_EXPORT void AudioApi_PlayObjSoundBgm(Vec3f* pos, s32 seqId) {
    ...
    if (pos != NULL) {
        ...
        if (sExtObjSoundMainBgmSeqId == NA_BGM_ASTRAL_OBSERVATORY) {
            ...
            SEQCMD_EXTENDED_PLAY_SEQUENCE(SEQ_PLAYER_BGM_MAIN, 10, 0, NA_BGM_CAVERN);
        } else {
